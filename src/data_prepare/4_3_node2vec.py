...
def node2vec():
    ...
    # load index_list_list
    file_index2length2id2index = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
    for file_index in file_index_list:
        for length in length_list:
            ...
            for id_index, id_ in enumerate(id_list):
                file_index2length2id2index[file_index][length][id_] = id_index
    ...
    for node_index, (node_sc, patent_id_list) in tqdm(enumerate(node_sc2patent_id.items())):
        file_index2length2index_list = defaultdict(lambda: defaultdict(list))
        for patent_id in patent_id_list:
            for file_index in file_index_list:
                for length in length_list:
                    if patent_id in file_index2length2id2index[file_index][length]:
                        file_index2length2index_list[file_index][length].append(
                            file_index2length2id2index[file_index][length][patent_id])
        vec_list = []
        for file in file_index2length2id2index:
            for length in file_index2length2id2index[file]:
                index_list = file_index2length2index_list[file][length]
                patent_feature = np.load(f'../../data/patent/inputs/doc2vec_{file}/patent_feature_{length}.npy')
                vec_list.extend(patent_feature[index_list].tolist())
